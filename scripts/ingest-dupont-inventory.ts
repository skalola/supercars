...
import { DuPontConnector } from "../lib/market-sources/connectors/dupont.connector";
...
async function ingestDupontListings(listings: NormalizedExternalListing[]) {
  ...
  console.log(`\nDuPont Registry Ingestion Summary:`);
...
async function main() {
  console.log("Starting DuPont Registry inventory ingestion (VIN-First)...");

  await cleanDatabase();

  const connector = new DuPontConnector();
  console.log(`Fetching from source: ${connector.sourceName}...`);
  
  try {
    const listings = await connector.fetchListings();
    console.log(`- Fetched ${listings.length} listings`);
    await ingestDupontListings(listings);
  } catch (e: any) {
    console.error(`- Error processing DuPont connector:`, e.message);
    process.exit(1);
  }
  
  console.log("DuPont Registry Ingestion finished successfully.");
}
