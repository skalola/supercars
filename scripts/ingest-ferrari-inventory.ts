...
async function ingestFerrariListings(listings: NormalizedExternalListing[]) {
...
async function main() {
  console.log("Starting Ferrari inventory ingestion (VIN-First)...");

  await cleanDatabase();

  const connector = new FerrariConnector();
  console.log(`Fetching from source: ${connector.sourceName}...`);
  
  try {
    const listings = await connector.fetchListings();
    console.log(`- Fetched ${listings.length} listings`);
    await ingestFerrariListings(listings);
  } catch (e: any) {
    console.error(`- Error processing Ferrari connector:`, e.message);
    process.exit(1);
  }
  
  console.log("Ferrari Ingestion finished successfully.");
}
