...
export async function GET(request: NextRequest, { params }: RouteParams) {
  ...
  return confirmationResponse({
    ...
    token,
    action: "ACCEPT",
    title: `Accept ${data.request.requestType.replaceAll("_", " ").toLowerCase()}`,
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  const result = await executePartnerDecisionByAction(token, "ACCEPT", note, {
...
function confirmationResponse(params: {
  ...
  const summary = escapeHtml(params.vehicleSummary || "Review the package before confirming.");
...
<html lang="en">
  <head>
    ...
    <style>
      ...
      button { border: 0; background: #16a34a; color: white; cursor: pointer; }
      ...
      <section>
        ...
        ${
          params.error
            ...
            : `<form method="post">
                <label>Partner note
                  <textarea name="note" placeholder="Optional confirmation details"></textarea>
                </label>
